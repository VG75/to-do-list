...
function addProject(projectName, projectContainer) {
    const div = document.createElement("div");
    div.classList.add("project-item"); // Add a class for easier identification
    div.setAttribute('data-project', `${projectName.toLowerCase()}`);
    div.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-list-check" viewBox="0 0 16 16">
                        ...
                        <button data-project="${projectName.toLowerCase()}">${projectName.charAt(0).toUpperCase() + projectName.slice(1)}</button>`;
